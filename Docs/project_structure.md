...
Conventions for the `flex/` crate. Read before running commands, creating
...
```text
flex/
  Cargo.toml               # pinned deps (ratatui =0.29.0), lints, release profile
  ...
  src/
    lib.rs                 # pub Row/Tab/Mode/Outcome/Theme/Provider/App
    ...
    providers.rs           # Provider trait + registry
    providers/
      power.rs
      launch.rs
      clip.rs
...
1. **Library never executes side effects.** `flex` as a library only produces an
...
2. **Binary prints exactly one `ACTION:` line** to stdout on success
   ...
   `1` = error (`src/backend.rs` owns this contract).
3. **Providers parse subprocess stdout once.** At most one child spawn per
   ...
   that. No re-spawning per keystroke. Two documented exceptions: `center`'s 1 s
   gauge tick re-reads `wpctl`/`brightnessctl` in place (Q7), and `wifi` opens
   from the cached scan and runs **one** background scan whose rows replace the
   list on a later tick (a triggered `nmcli` scan blocks ~3 s, which would leave
   the popup blank until it returned).
3b. **Image previews are the one out-of-band surface (`src/preview.rs`).**
...
5. **`target/` gitignored + stow-ignored.** Entries required in both
   ...
   artifacts. `Cargo.lock` IS committed (Q5).
...
8. **Lints deny by default.** `[lints] unsafe_code deny; clippy all+pedantic deny`
   in `Cargo.toml`; `cargo clippy --all-targets -- -D warnings` must pass.
9. **Formatting:** `cargo fmt --check` green; `rustfmt.toml max_width=100`.
...
- `cargo test` (all suites incl. `tests/golden.rs`)
- `cargo tree -i crossterm` (single-major check)
- `cargo bench --bench rerank` (M5)
- Negative-dependency gate: `! rg -l '"serde"|"toml"' src tests benches Cargo.toml`
