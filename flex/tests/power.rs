...
use flex::{backend, run, width, Menu};

fn power_menu() -> Menu {
    Menu::new(power::PROVIDER, vec![power::power_tab()])
}
...
fn armed_confirmable_row_golden_at_80x24() {
    ...
    let metrics = flex::render::node_metrics(&menu);
    assert_eq!(metrics, flex::render::NodeMetrics::COMPACT);
    let header = flex::render::LIST_INDICATOR_ROWS / 2 + 2 * metrics.pitch();
