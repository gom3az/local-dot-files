...
use flex::providers::launch;
use flex::{backend, run, width, Menu, Row, RowId, Tab};
...
fn fixture_menu() -> Menu {
    ...
    Menu::new(launch::PROVIDER, vec![tab])
}
...
fn legacy_filter_mode_preserves_provider_order() {
    ...
    menu.app.filter_mode = flex::filter::FilterMode::Legacy;
    assert_eq!(
        menu.app.visible_rows(),
        vec![0, 1, 2],
