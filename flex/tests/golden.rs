...
use flex::providers::power;
use flex::{render, width, Gauge, Menu, Peaks, Row, RowId, RowPeaks, Tab, Target, Theme};
...
fn bare_rows_mode_has_no_chrome_and_no_indicators() {
    ...
    tab.bare_rows = true;
    let mut menu = Menu::new("launch", vec![tab]);
    let buf = draw(&mut menu, 80, 24);
    assert_full_width(&buf);
    let mut all = String::new();
    for y in 0..24 {
...
fn power_tab_has_launch_style_chrome_at_80x24() {
    let mut menu = Menu::new(power::PROVIDER, vec![power::power_tab()]);
    let buf = draw(&mut menu, 80, 24);
    assert_full_width(&buf);
    let mut all = String::new();
    for y in 0..24 {
...
fn degenerate_frames_never_panic_and_stay_exact() {
    let mut empty = Menu::new("test", vec![]);
...
fn charset_and_theme_switch_the_rendering() {
    use flex::{CharSet, CharSetName, ThemeName};
